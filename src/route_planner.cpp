...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    RouteModel::Node* node_it = current_node;
    path_found.push_back(*node_it);
    while(node_it!= start_node){
        distance += node_it->distance(*node_it->parent);
...
void RoutePlanner::AStarSearch() {
    ...
    // Search until end_node is reached or list of elements to search (open_list) is empty.
    while((current_node != end_node)&&(current_node != nullptr)){
        // Find neighbours for current node.
        AddNeighbors(current_node);

        // Find next node, which has the smallest (g+h) value.
